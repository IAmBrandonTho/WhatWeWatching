...
on(audioPick,'change', async () => {
  ...
  async function loadVideoFile(file){
    try{
      ...
      const okForSeg = (typeof window !== 'undefined') && window.FFmpeg && window.FFmpeg.createFFmpeg;
      if(okForSeg){
        await hostUploadSegmentedAndPlay(file);
        return;
      }

      addLog('FFmpeg.wasm not loaded; falling back to legacy /upload (no progressive seek).', 'warn');
...
const FFCORE_FALLBACK = "/ffmpeg/ffmpeg-core.js";
...
async function getFfmpeg(){
  ...
  _ffmpegLoading = (async ()=>{
    ...
    const ffGlobal = (window.FFmpeg && window.FFmpeg.createFFmpeg) ? window.FFmpeg
      : (window.FFmpegWASM && window.FFmpegWASM.createFFmpeg) ? window.FFmpegWASM
      : null;
    const createFFmpeg = ffGlobal?.createFFmpeg || window.createFFmpeg;
    if(!createFFmpeg){
      throw new Error('FFmpeg.wasm not available (createFFmpeg missing). Make sure /ffmpeg/814.ffmpeg.js is served correctly.');
    }

    // Single-thread core (no SharedArrayBuffer required)
    let ffmpeg;
    addLog("Loading FFmpeg.wasm core (first time can take ~30MB)...", "warn");
    try{
      ffmpeg = createFFmpeg({ log: true, corePath: FFCORE_PRIMARY });
      await ffmpeg.load();
    }catch(e1){
      addLog("FFmpeg core failed from primary CDN, retrying fallback…", "warn");
      ffmpeg = createFFmpeg({ log: true, corePath: FFCORE_FALLBACK });
      await ffmpeg.load();
    }
...
async function segmentWithFmp4Hls(ffmpeg, file, segSec){
  const fetchFile = (window.FFmpeg && window.FFmpeg.fetchFile) ? window.FFmpeg.fetchFile : (window.FFmpegWASM && window.FFmpegWASM.fetchFile) ? window.FFmpegWASM.fetchFile : null;
  if(!fetchFile) throw new Error("FFmpeg fetchFile missing (FFmpeg not loaded)");
  ...
  for(let i=0;i<200000;i++){
    ...
    try{ ffmpeg.FS("stat", name); safeUnlink(name); }catch{ break; }
  }
...
async function probeInputCodecs(ffmpeg, file){
  ...
  // Returns { videoCodec, audioCodec } such as { h264, aac } when detectable.  const fetchFile = (window.FFmpeg && window.FFmpeg.fetchFile) ? window.FFmpeg.fetchFile : (window.FFmpegWASM && window.FFmpegWASM.fetchFile) ? window.FFmpegWASM.fetchFile : null;
  if(!fetchFile) throw new Error("FFmpeg fetchFile missing (FFmpeg not loaded)");

  // Write to FS temporarily
  try{ ffmpeg.FS("unlink", "probe"); }catch{}
  ffmpeg.FS("writeFile", "probe", await fetchFile(file));
  ...
  try{
    ...
    try{
      await ffmpeg.run("-hide_banner","-nostdin","-i","probe","-f","null","-");
    }catch{}
  }finally{
    if(ffmpeg.setLogger){
  ...
  return { videoCodec: v ? v[1].toLowerCase() : "", audioCodec: a ? a[1].toLowerCase() : "" };
}
...
async function segmentWindowHlsFmp4(ffmpeg, mode, segSec, startSeg, countSeg){
  ...
  const dur = Math.max(segSec, countSeg * segSec);

  // Clean outputs for this run (best-effort)
  try{ ffmpeg.FS("unlink", "out.m3u8"); }catch{}
  ...
  if(mode === "copy"){
    await ffmpeg.run(...common, "-c", "copy", ...hlsArgs);
    return;
  }
...
async function putInitFile(uploadBase, videoId, ffmpeg, initName, mime){
  ...
  const initBuf = ffmpeg.FS("readFile", initName);
...
async function putSegmentRangeFromFs(uploadBase, videoId, ffmpeg, startSeg, countSeg, opts){
  ...
  for(let i=startSeg;i<startSeg+countSeg;i++){
    const name = `seg-${pad6(i)}.m4s`;
    try{
      const bytes = ffmpeg.FS("readFile", name);
...
async function hostUploadSegmentedAndPlay(file){
  ...
  // Upload init.mp4
  await putInitFile(uploadBase, videoId, ffmpeg, "init.mp4", mime);

  // Upload the initial window segments (helps first-play)
  await putSegmentRangeFromFs(uploadBase, videoId, ffmpeg, initWindow.startSeg, initWindow.countSeg, {
    concurrency: 4,
    deleteAfter: true
  });
  ...
  async function startSegmentedPlayback(payload){
    ...
    const append = (ab) => new Promise((resolve, reject)=>{
    ...
    await append(await initRes.arrayBuffer());
    ...
    const ensureBufferLoop = async ()=>{
      while(segmentedCtl && segmentedCtl.videoId === videoId){
        // Target ~25s ahead
        const ahead = 25;
        const bufferedEnd = getBufferedEnd(video.currentTime || 0);
        if(bufferedEnd - (video.currentTime || 0) < ahead && !sb.updating){
          ...
          if(r.ok){
            ...
            // Segment not uploaded yet -> wait a bit then retry
