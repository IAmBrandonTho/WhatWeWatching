  ...
  try{
    ...
    pc.addEventListener("track",(ev)=>{
      ...
      if(stream){
        ...
        try{__pwForceAutoplayMute('viewer-track');}catch{}
        try{__pwSetAudioControlsEnabled(true);}catch{}
...
function Kr(){
  try{
    ...
    // Audio controls are only enabled when an actual stream is active/received.
    try{
      const enabled = (nt==='host') ? !!__pwStreamIsLive : (nt==='viewer' ? __pwViewerHasLiveStream() : false);
      __pwSetAudioControlsEnabled(enabled);
    }catch{}
  }catch(e){}
}
...
async function __pwStartStreamingWithOptions(opts){
  ...
  try{__pwForceAutoplayMute('host-start-stream');}catch{}
...
function __pwStopStreaming(){
  ...
  __pwStopDrawLoop();
  try{__pwStopHostTimeTicker();}catch{}
  try{__pwLastDur=0;__pwDurSent=false;}catch{}
  try{__pwSetAudioControlsEnabled(false);}catch{}
  try{
    for(const [pid,peer] of gt.entries()){const pc=peer.pc;
      const senders = pc?.getSenders?.()||[];
...
function __pwWireStartButton(){
  ...
  b.addEventListener("click", async ()=>{
    ...
    if(!__pwStreamIsLive){
      ...
      try{__pwForceAutoplayMute('host-start-btn');}catch{}
...
/* === PATCH: enforce mute-before-load + viewer stability === */
(function(){
  const setup = () => {
    ...
    // Always start muted BEFORE any media attaches (autoplay compliance)
    try{
      v.muted = true;
      v.volume = 0;
    }catch{}

    const disableAudioUI = () => {
    ...
    const enableAudioUI = () => {
      ...
      // remain muted initially; user must unmute manually
      try{ v.muted = true; }catch{}
    };

    disableAudioUI();

    // Detect when stream actually becomes active
    const checkStream = () => {
      const active = v.srcObject &&
        v.srcObject.getTracks &&
    ...
    // Watch for stream attach/change
    const obs = new MutationObserver(checkStream);
    obs.observe(v, {attributes:true, attributeFilter:["srcObject"]});

    v.addEventListener("loadedmetadata", checkStream);
    v.addEventListener("emptied", disableAudioUI);
    setInterval(checkStream, 2000);

    // Prevent premature mute toggles
    if(muteBtn){
    ...
    // Viewer click behavior: play if paused only
