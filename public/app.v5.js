  ...
  if(S && !S.__pwClickFs){
    ...
    S.addEventListener("click",(ev)=>{
      try{
        ...
        __pwClickT=setTimeout(()=>{ __pwClickT=null; try{ if(S.paused) __pwSafePlay(S); else __pwSafePause(S);}catch{} },220);
      }catch{}
    });
...
function makePeer(opts){
  ...
  try{
    ...
    pc.addEventListener("track",(ev)=>{
      ...
      if(stream){
        try{ S.srcObject=stream; S.autoplay=true; S.playsInline=true; S.volume=Number(App.audio.viewerVol??0.8)||0.8; S.muted=!App.audio.viewerUnmuted; }catch{}
